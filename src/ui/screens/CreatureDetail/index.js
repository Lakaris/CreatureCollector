...
function CreatureDetail({ownedData,onBack,onEvolve,onBananaUsed,onCandyUsed,onSwipeNav}){
          ...
          (()=>{
            ...
            return[...EQUIPMENT_DEFS]
              .filter(item=>{
                ...
                if(equipFilterHideIncompatible&&!equipped.includes(item.id)&&!itemFitsCreature(item,def))return false;
                return true;
              })
              .sort((a,b)=>{
  const ea=equipped.includes(a.id)?1:0,eb=equipped.includes(b.id)?1:0;if(eb!==ea)return eb-ea;
  const dA=itemFitsCreature(a,def)?0:1;
  const dB=itemFitsCreature(b,def)?0:1;
  ...
  return RARITY_RANK[b.rarity]-RARITY_RANK[a.rarity];
            ...
            const disabled=!itemFitsCreature(item,def);
            ...
            return React.createElement("div",{key:item.id,
              ...
              React.createElement("div",{style:{position:"absolute",top:6,left:8,textAlign:"left"}},
                ...
                isExclusive(item)&&React.createElement("div",{style:{fontSize:10,fontWeight:700,color:"#7F77DD",marginTop:2,whiteSpace:"nowrap"}},
  ...
  return React.createElement("div",swipeHandlers,
    ...
    confirmMelon&&React.createElement("div",{className:"screen-fade",style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100}},
      ...
      React.createElement("div",{style:{position:"relative",marginBottom:14}},
        ...
        React.createElement("div",{style:{position:"absolute",top:0,right:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:3}},
          ...
          (()=>{
            ...
            return React.createElement(AscStars,{n:asc.count,max:5,doubled:asc.doubled,colors:asc.colors,slotted:true,style:{fontSize:11,gap:1,...asc.style}});
          })()
        )
      ),
      React.createElement(StatStrip,{values:statsWithEquip,onClick:setStatInfoPopup,isHighlighted:s=>lastLeveledStat===s})
    ...
    React.createElement("div",{style:{display:"flex",gap:4,marginBottom:12,background:"#ebebeb",borderRadius:10,padding:4}},
      tabs.map(t=>{
        ...
        const showCandyGuideArrow=candyGuideStep==="candySkins"&&t.id==="skins";
        return React.createElement("button",{
          key:t.id,
          disabled:tabLocked,
          "data-guide-target":t.id==="flair"?"flair":t.id==="skins"?"candySkins":undefined,
          onClick:()=>{if(tabLocked)return;setTab(t.id);if(flairGuideStep==="flair"&&t.id==="flair")setFlairGuideStep("feed");if(candyGuideStep==="candySkins"&&t.id==="skins")setCandyGuideStep("candyFeed");},
          ...
          (showFlairGuideArrow||showCandyGuideArrow)&&React.createElement("div",{style:{position:"absolute",left:"50%",top:-30,transform:"translate(-50%,0)",fontSize:22,color:"#534AB7",animation:"pointerBounce 1s ease-in-out infinite",zIndex:6,pointerEvents:"none",filter:"drop-shadow(0 2px 4px rgba(0,0,0,0.25))"}},"⬇️"),
          t.label
        );
      })
    ),
    ...
    tab==="skins"&&React.createElement(SkinSection,{
      ...
      onCandyUsed,candyGuideStep,setCandyGuideStep
    })
  );
}
